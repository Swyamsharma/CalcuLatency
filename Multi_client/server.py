...
import json
import config
# Tracer for 0trace logic, callback, and sniffer runner
...
async def handle_connection(websocket, path):
    ...
    client_log_prefix = f"[{client_ip}:{client_port}]" # Prefix for client-specific logs
    ...
    try:
        ...
        # Run 0trace
        local_tracer = Tracer() # Create a local Tracer for this connection
        ...
        with ThreadPoolExecutor() as pool:
             trace_rtt_ms, trace_hop_ip, trace_success = await loop.run_in_executor(
                 pool,
                 local_tracer.measure_0trace_rtt, # Use local tracer instance method
                 server_ip_local,
                 server_port_local,
                 client_ip,
                 client_port,
...
async def main():
    ...
    sniffer_thread = threading.Thread(
        ...
        args=(
            iface,
            server_ip,
            tracer_callback, # Pass module-level callback
    ...
    try:
        ...
        async with websockets.serve(handle_connection, "0.0.0.0", config.SERVER_PORT): # Pass handler directly
